...
name: start-3-1-1
description: |
  Module 3.1.1: Welcome & First Generation - Set up Gemini API and generate your first image with Nano Banana. Use when the student types /start-3-1-1.
...
```bash
cp -rn .cursor/skills/start-3-1-1/assets/* . 2>/dev/null || true
```
...
L="cursor-pms-3-1-1"; C="cursor-pms-3-1-2"; T="$(date -u +%FT%TZ)"
