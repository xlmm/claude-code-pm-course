...
name: start-1-2
description: |
  Module 1.2: The Cursor Interface - Learn the three-pane layout. Use when the student types /start-1-2.
...
```bash
cp -rn .cursor/skills/start-1-2/assets/* . 2>/dev/null || true
```
...
L="cursor-pms-1-2"; C="cursor-pms-1-3"; T="$(date -u +%FT%TZ)"
