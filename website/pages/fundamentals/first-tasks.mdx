...
| **Image Pasting** | Paste screenshots directly into Claude (Ctrl+V in the CLI on all platforms) |
...
> **Using the Claude Desktop app?** Paste normally: `Cmd+V` on Mac, `Ctrl+V` on Windows. The rule below is for the CLI, where terminals use `Ctrl+V` on every platform.
