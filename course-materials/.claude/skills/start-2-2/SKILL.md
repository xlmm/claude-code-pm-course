...
```bash
cp -rn ".claude/skills/start-2-2/assets/." .
```
...
I can read the CSVs and will present the useful results as tables in this chat. If you open a raw CSV from the Desktop project tree, treat it as source data; you do not need another app for this lesson."
...
"Done! I've created [activation-problem-analysis.md](activation-problem-analysis.md) with our complete analysis.

You can open the linked Markdown report in Desktop to see the full document. It includes:
...
"Here's the impact estimation framework: [impact-estimation-framework.md](impact-estimation-framework.md).
...
The full framework document has examples and a template. Open [impact-estimation-framework.md](impact-estimation-framework.md) and skim it before we continue.
...
"Done! I've created [onboarding-experiment-readout.md](onboarding-experiment-readout.md) with the complete analysis.
...
Open [onboarding-experiment-readout.md](onboarding-experiment-readout.md) to see the full analysis with all the supporting data.
...
- If students ask about raw CSVs, explain that the agent reads them and presents useful results as chat tables; no other app is required.
...
```bash
fspm progress complete cc-pms-2-2
```
...
**Next lesson:** [`/start-2-3`](.claude/skills/start-2-3/SKILL.md)
